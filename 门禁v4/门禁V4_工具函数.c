...
{
    if (check_string_length(str, 11, 11) == 0)
        return 0;
    for (int i = 0; str[i] != '\0'; i++)
    {
        if (!(str[i] >= '0' && str[i] <= '9'))
...
// 读取文件
void load_users(char *file, Node *head)
{
    ...
    while (fgets(line, sizeof(line), fp))
    {
        ...
        sscanf(line, "%s %s %s %s %s %c %c %c %ld",
               u->username, u->passwrod, u->realname, u->phone, u->address,
               &u->role, &u->status, &u->lock, &u->lock_time);
        add_node(head, u); // 添加到链表
    }

    fclose(fp);
}
// 读取日志
void load_logs(char *file, mlog *log)
{
    FILE *fp = fopen(file, "r");
    if (fp == NULL)
    {
    ...
    {
        line[strcspn(line, "\n")] = 0; // 去掉换行符
        log_add(log, strdup(line));    // 保存到动态数组
    }
...
// 写入文件
void save_users(char *file, Node *head)
{
    ...
    {
        User *u = temp->user;
        fprintf(fp, "%s %s %s %s %s %c %c %c %ld\n",
                u->username, u->passwrod, u->realname, u->phone,
                u->address, u->role, u->status, u->lock, u->lock_time);
        temp = temp->next;
    }
    fclose(fp);
}
// 写入日志
void save_logs(char *file, mlog *log)
{
    FILE *fp = fopen(file, "w");
    if (fp == NULL)
    {
