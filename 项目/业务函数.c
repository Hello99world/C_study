...
int car_run(char *card, int type)
{
    ...
    if (type == 3)
    { // 临停车查找空闲车位
        if(search("月租车辆.txt", card) != NULL || search("长期车辆.txt", card) != NULL){
            printf("本车已存在\n");
            return -1;
        }
        if (free_space(num) != 0)
